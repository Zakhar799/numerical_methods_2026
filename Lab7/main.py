...
def simple_iteration(A, B, X_true, epsilon, max_iter=20000):
    n = len(B)
    norm_A = matrix_norm(A)
    tau = 1.0 / norm_A

    X = np.ones(n)
    errors = [vector_norm(vector_diff(X, X_true))]

    for k in range(max_iter):
        X_new = np.zeros(n)
        for i in range(n):
            ax_sum = sum(A[i][j] * X[j] for j in range(n))
            X_new[i] = X[i] - tau * ax_sum + tau * B[i]

        current_error_true = vector_norm(vector_diff(X_new, X_true))
        errors.append(current_error_true)

        if vector_norm(vector_diff(X_new, X)) < epsilon:
...
def jacobi(A, B, X_true, epsilon, max_iter=20000):
    n = len(B)
    X = np.ones(n)
    errors = [vector_norm(vector_diff(X, X_true))]
...
def seidel(A, B, X_true, epsilon, max_iter=20000):
    n = len(B)
    X = np.ones(n)
    errors = [vector_norm(vector_diff(X, X_true))]
...
err_si = vector_norm(vector_diff(X_si, X_exact))
...
err_jacobi = vector_norm(vector_diff(X_jacobi, X_exact))
...
err_seidel = vector_norm(vector_diff(X_seidel, X_exact))
...
plt.subplot(1, 2, 1)
plt.plot(errors_si, label='Метод простої ітерації', color='blue', linestyle='-')
...
plt.ylabel("Похибка (log)", fontsize=12)
